...
void writeLine(FILE* output, size_t dim, char info[DIM][MAX_LONG]){ //Se escribe una línea dentro del archivo dado con los datos que haya en la matriz indicada por parámetro
    int i;
    if(dim>DIM)
        return;
    for(i=0 ; i < dim-1 ; i++)
    ...
    fprintf(output, "%s\n", info[i]);                         //Como es el último se añade un ('\n')
}
